...
![OBS にオーバーレイした StreamClock](assets/screenshot.png)

> **エディション** — どちらも**無料**:
> - **フル版**（Windows / macOS 直接配布・Homebrew）: System / NTP / PTP / MTC / LTC。
> - **ライト版**（macOS App Store・予定）: 入力時刻ソースを **System + NTP に限定**し、Sandbox に
>   適合させて審査を通すための版。App Store 版は署名済みのため、**Gatekeeper の警告が出ずに
>   インストール**できます。LTC/MTC タイムコード出力は利用できます（直接配布のフル版は未署名で、
>   下記の回避が必要です）。
>
...
（PTP/MTC/LTC の**入力**はフル版のみ）

## タイムコード出力（LTC / MTC）

Settings の **Timecode Output** から、現在表示している時刻ソースを LTC（選択したオーディオ出力）と
MTC（選択した MIDI 出力）として送出できます。LTC と MTC は同時使用でき、フレームレート設定を共有します。
macOS で動作確認済みです。
...
- **タイムコード出力**: 共通レート、LTC オーディオ出力デバイスとレベル、MTC MIDI 出力ポート。
...
src/
  ...
  mtc_out.rs — MTC 送信 (midir)      [tc-out フィーチャー]
  ltc_out.rs — LTC 生成・音声出力 (cpal) [tc-out フィーチャー]
...
**ライト版 / App Store** ビルドは
`cargo build --release --no-default-features --features tc-out` で生成します。PTP/MTC/LTC の入力を
コンパイル時に除外し、再生/CoreMIDI による LTC/MTC 出力のみを有効にします。
