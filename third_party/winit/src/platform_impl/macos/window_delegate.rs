...
impl WindowDelegate {
    ...
    pub fn set_blur(&self, _blur: bool) {
        // PATCHED FOR THE MAC APP STORE (StreamClock).
        //
        // Upstream winit implements window background blur via the private
        // CoreGraphics Services symbols `CGSSetWindowBackgroundBlurRadius` and
        // `CGSMainConnectionID`. App Review rejects any binary that merely
        // *references* those symbols (Guideline 2.5.1 — non-public API), even
        // when the code path is never taken at runtime, because it flags the
        // undefined imports statically. StreamClock never requests blur
        // (`WindowAttributes::blur` stays false), so this whole path is dead
        // for us; we neutralize it to a no-op to keep those private symbols out
        // of the linked binary. See third_party/winit/PATCH.md.
        //
        // If a real blur is ever needed, do it with public API
        // (NSVisualEffectView) instead of restoring the CGS calls.
    }
