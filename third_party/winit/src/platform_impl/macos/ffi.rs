...
// NSInteger / AnyObject were only used by the private CGS blur externs removed
// below for Mac App Store compliance. See third_party/winit/PATCH.md.
...
extern "C" {
    ...
    // PATCHED FOR THE MAC APP STORE (StreamClock): the private CoreGraphics
    // Services symbols `CGSMainConnectionID` and `CGSSetWindowBackgroundBlurRadius`
    // (used only by WindowDelegate::set_blur, which we neutralized) are removed
    // here so they are not linked into the binary. App Review Guideline 2.5.1
    // rejects any reference to non-public APIs. See third_party/winit/PATCH.md.
    // `CGShieldingWindowLevel` above is intentionally kept: it is public API
    // (declared in <CoreGraphics/CGWindowLevel.h>) and is not flagged.
}
